...
\maketitle
\begin{abstract}
Because micro and nano robots are small enough to go toward the passageways of the body, they are suited for targeted drug delivery and micro scale manufacturing. But due to their small size robots cannot contain onboard processing for autonomy nor onboard power. Instead they are controlled by external signal such as a magnetic field. However, because each robot can only provide a small amount of force or transport a small amount of material, we need large swarms of robots, all controlled by the same external field. This work presents controllers and algorithms for steering such an under-actuated swarm. This paper has two main components. The first proves that the mean position of the swarm is controllable, and show how an obstacle can make the variance controllable. Then it derives automatic controllers for these and a hybrid, hysteresis-bases switching control to regulate the first two moments of the robot distribution. Second paper uses friction with boundary walls to break the symmetry caused by the global input and uses it to steer two robots to arbitrary positions. Finally, to show the results of those two paper on hardware robots, we use all these techniques in our experiments. We employ these controllers as primitives for a block pushing task and covariance control of a swarm of 64 robots.
\end{abstract}
%%%%%%%%%%% PAPER OUTLINE
...
%\bibliography{IEEEabrv,QualifyingExam,../../../RoboticSwarmControlLab/bib/aaronrefs}
\bibliography{QualifyingExam}
