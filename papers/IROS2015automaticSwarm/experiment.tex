...
Figure~\ref{fig:story} shows snapshots during an execution of this algorithm, and experimental results are summarized in Fig.~\ref{fig:AutoControlVaryN}.  Minimizing the variance requires more time with larger numbers of robots, and so the algorithm is slower for larger numbers of robots
...
\end{overpic}
\vspace{-1em}\caption{Algorithm \ref{alg:BlockPushing} fails when some robots are separated by the maze and the swarm can not achieve $\sigma^2 < \sigma_{min}^2$.  These failures occur during 14\% of trials.\label{fig:Failure}
%\vspace{-2em}
}
\end{figure}
