...
\end{align}
where $K_{p}$ is the proportional gain, and $K_{d}$ the derivative gain. We performed a parameter sweep to identify the best values.  Representative experiments are shown in Fig.~\ref{}.  100 robots were used and the maximum speed was 3 cell per second. \todo{what is 'cell per second'?} With these parameters, we showed that the best values are  $K_{p}= 4$, and  $K_{d} = 1$.
