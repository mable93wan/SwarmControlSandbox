...
\end{align}
In a slight abuse of notation, because the variance integrates over time, we call the gain scaling the variance error $K_i$.
\eqref{eq:PDcontrolVariance} assumes that the nearest wall is to the left of the robot at $x=0$, and chooses a reference goal position that in steady-state would have the correct variance according to \eqref{eq:VarianceUniformDistribution}:
\begin{align}
