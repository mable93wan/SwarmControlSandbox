...
\begin{figure}
\centering
%\begin{overpic}[width=\columnwidth]{blockpushingImageWithMeanAndVarianceOverlay.png}\end{overpic}
\todo{put a pretty picture of robots pushing a block.  Also include the mean and variance ellipses}
%{blockpushingImageWithMeanAndVarianceOverlay.png}
\caption{\label{fig:bigPictureMeanAndVarianceForSwarm} A swarm of robots, all controlled by global inputs consisting of a vector heading, can be effectively controlled by a controller knowing the first and second moments of the robot distribution.  Here a swarm of simple robots pushes a block toward the goal.}
\end{figure}
