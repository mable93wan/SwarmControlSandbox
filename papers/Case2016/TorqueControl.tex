% TODO: these are the wrong links for CASE 2016:  http://www.iros2015.org/
...
\begin{abstract}
Manipulating objects with a swarm of simple robots with global control inputs is difficult because they are highly under actuated, the number of robots in contact with object changes dynamically, the robots contacting each other changes dynamically, and the robots disperse over time and must be recollected.
...
However, due to their small size, robots cannot contain onboard processing for autonomy nor onboard power. 
...
Previous work showed that mean and variance of the swarm is controllable. This was used to manipulate polygonal objects through a simple maze. A key remaining challenge is controlling the torque applied to an object. 
Torque control is necessary for manipulating objects as well as placing sensors and exerting forces.
This work first proves that swarm torque control is possible, then presents algorithms to automate the task of position control of the object with the swarm.
The paper concludes with experimental results using 100 hardware robots to manipulate rectangles with large aspect ratios.
