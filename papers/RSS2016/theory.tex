...
Assume an open workspace with boundary friction. The wall has infinite friction for simplicity. Variance($\sigma_x^2$) in $x$ axis would shrink if the swarm hits the vertical walls, and variance($\sigma_y^2$) in $y$ axis would shrink if the swarm hits the horizontal walls. Alg. \ref{alg:CovCont} works as follows:
As the first step, the robots are moved to the left wall to shrink $\sigma_x^2$. After the robots reach a comparatively smaller $\sigma_x^2$ than the goal variance, they will go to the center of the workspace. This is because after they hit the horizontal wall, their $\sigma_x^2$ will increase and it will keep them from growing highly in $\sigma_x^2$. Then the robots will move to the bottom wall until they reach the desired $\sigma_y^2$. The last step is siding the robots to left or right depending on the sign of the desired covariance to make the covariance needed. At the end robots return to the center to avoid any other influence of the high friction boundary. 

\begin{algorithm}
\caption{Swarm Covariance Control With Wall Friction}\label{alg:CovCont}
\begin{algorithmic}[1]
\Require Knowledge of the goal variances and covariance$(\sigma_{goalx}^2,\sigma_{goaly}^2, \sigma_{goalxy})$ and mean, variances and covariance of the swarm$(\bar{x},\bar{y},\sigma_x^2,\sigma_y^2, \sigma_{xy})$ .Swarm is Gaussian Distributed(in limit it is uniformly distributed), $L_w$ is width of the walls and $L_h$ is the height of the wall. $(0,0)$ is top right corner. $\epsilon$ is the error for reaching covariance.
